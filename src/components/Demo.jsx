...
const Demo = () => {
  ...
  useEffect(()=>{
    const articlesFromLocaalStorage= JSON.parse( localStorage.getItem('articles'))
    
    if(articlesFromLocaalStorage) {
      setAllArticles(articlesFromLocaalStorage)
    }
  ...
  const handleCopy = (copyUrl)=>{
    setCopied(copyUrl);
    navigator.clopboard.writeText(copyUrl);
    ...
    <section className='mt-16 w-full max-w-xl'>
      <div className='flex flex-col w-full gap-2'>
        <form className='relative flex justify-center items-center' onSubmit={handelSubmit}>
          ...
          <input type='url' placeholder='Enter a URl' value={article.url} onChange={(e)=>setArticle({article, url:e.target.value})} className='url_input peer'/>
